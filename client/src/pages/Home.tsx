import { ArrowRight, EyeOff, MapPin, MessagesSquare, Sparkles } from "lucide-react";
...
const moments = [
  { icon: EyeOff, title: "Lead with what feels like you", copy: "A chosen name, a short introduction, and only the details you want to share." },
  { icon: MapPin, title: "Stay close to your city", copy: "Browse local possibilities without turning your presence into a precise map pin." },
...
export default function Home() {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#080d1d] text-white">
    <SiteHeader />
    <main>
      <section className="relative overflow-hidden border-b border-white/8">
        ...
        <div className="container relative grid min-h-[660px] items-center gap-10 py-14 lg:grid-cols-[.85fr_1.15fr] lg:py-20">
          <div className="max-w-xl">
            ...
            <h1 className="mt-5 font-serif text-4xl leading-[1.02] tracking-[-.035em] sm:text-5xl lg:text-6xl">A more interesting way to meet <span className="text-[#e1c687]">nearby.</span></h1>
            <p className="mt-6 max-w-lg text-base leading-8 text-slate-300 sm:text-lg">v3rya brings adult listings, local energy, and unhurried conversation into one refined place.</p>
            <div className="mt-9 flex flex-wrap gap-3">
              <Button onClick={() => isAuthenticated ? setLocation("/post") : startLogin()} className="h-12 rounded-xl bg-[#e1c687] px-6 font-semibold text-[#10172c] hover:bg-[#f0da9f]">{isAuthenticated ? "Create a listing" : "Enter v3rya"}<ArrowRight className="ml-2 h-4 w-4" /></Button>
      ...
      <section className="container py-16 sm:py-24">
        <div className="grid items-end gap-8 lg:grid-cols-[.8fr_1.2fr]">
          <div><p className="eyebrow">Make the first move your own</p><h2 className="mt-4 max-w-md font-serif text-3xl leading-tight sm:text-4xl">An introduction can be as simple as a good feeling.</h2></div>
