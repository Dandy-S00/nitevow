...
async function startServer() {
  ...
  app.get("/sitemap.xml", (req, res) => { const origin = originFor(req); const publicPaths = ["/", "/browse", "/safety"]; const urls = publicPaths.map(path => `<url><loc>${origin}${path}</loc><changefreq>weekly</changefreq><priority>${path === "/" ? "1.0" : path === "/browse" ? "0.8" : "0.6"}</priority></url>`).join(""); res.type("application/xml").send(`<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${urls}</urlset>`); });
